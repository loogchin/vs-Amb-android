...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			case 'almondbon-sus':
				...
				var brothisreal:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('folder/thisisreal', 'shared'));
				brothisreal.antialiasing = true;
				brothisreal.scrollFactor.set(0.9, 0.9);
				brothisreal.active = false;
				add(brothisreal);

				var realbg:FlxSprite = new FlxSprite(-50, -50).loadGraphic(Paths.image('folder/qwert', 'shared'));
				realbg.antialiasing = true;
				realbg.scrollFactor.set(0.9, 0.9);
				realbg.active = false;
				add(realbg);
			case 'bonless':
				defaultCamZoom = 0.9;
				...
				var funsky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('folder/sonicFUNsky'));
				...
				var funfloor:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('folder/sonicFUNfloor'));
			...
			case 'smile':
				...
				var brothisreal:FlxSprite = new FlxSprite(-600, -275).loadGraphic(Paths.image('folder/street2', 'shared'));
				brothisreal.antialiasing = true;
				brothisreal.scrollFactor.set(0.9, 0.9);
				brothisreal.active = false;
				add(brothisreal);

				dad = new Character(0, 0, 'laugh');
				dad = new Character(0, 0, 'true');
		}
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			...
			switch (SONG.player2)
			{
				case 'amblookingkindasus':
					{
						p2ColorBar.setRGB(159, 121, 99, 255);
					}

				case 'majinamb':
					{
						p2ColorBar.setRGB(44, 23, 87, 255);
					}
		...
		var portby:FlxText = new FlxText(0,0,0,"Port by Nong vanila", 72);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		{
			if (daSection == 58 && curSong.toLowerCase() == 'bonless')
				SONG.noteStyle = 'bonnote';
	...
	var docamshake:Bool = false;

	override public function update(elapsed:Float)
	{
		if (docamshake)
		{
			FlxG.camera.shake(0.075, 0.075);
		}
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								...
								if (curSong == 'almondbon-sus' || curSong == 'bonless')
								{
									health -= 0.01;
								}
							case 3:
			...
			if (isStoryMode)
				{
						...
						if (storyPlaylist.length <= 0)
						{
							...
							{
								case 'almondbon-sus':
									LoadingState.loadAndSwitchState(new VideoState('assets/videos/bfisscary.webm', new MainMenuState()));
								default:
									FlxG.sound.playMusic(Paths.music('FullMenu'));
	...
	*/
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'majinamb' && SONG.song.toLowerCase() == 'bonless')
			{
				switch (curStep)
				{
					case 10:
			...
			if (SONG.song.toLowerCase() == 'almondbon-sus')
				switch (curStep)
				{
					case 65:
						docamshake = true;
			...
			if (SONG.song.toLowerCase() == 'smile')
				switch (curStep)
				{
					case 440:
						remove(dad);
						dad = new Character(0, 100, 'laugh');
						add(dad);
					case 448:
						remove(dad);
						dad = new Character(0, 385, 'true');
						add(dad);
						docamshake = true;
				}
